  ...
  <div class="h-full flex flex-col gap-4 min-h-0">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-4 flex items-center gap-6 flex-shrink-0">
      ...
      <div class="flex items-center gap-3 flex-shrink-0">
        ...
        <div>
          <p class="text-xs text-gray-400">我的审核进度</p>
      ...
      <div class="flex items-center gap-4 flex-1 justify-end min-w-0">
        ...
        <button class="flex-shrink-0 px-4 py-2 border border-gray-200 rounded-xl text-sm text-gray-600 hover:bg-gray-50 transition-colors font-medium">批次详情</button>
    ...
    <div class="flex-1 grid grid-cols-12 gap-4 min-h-0">
          ...
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="flex items-center gap-0.5 bg-gray-100 rounded-lg p-0.5">
              <button
                ...
                :key="tab.value"
                @click="activeFundTab = tab.value"
                class="px-2.5 py-1 rounded-md text-xs font-semibold transition-all"
                :class="activeFundTab === tab.value ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'"
              >{{ tab.label }}</button>
            </div>
          </div>
          <!-- 搜索框 -->
      ...
      <div class="col-span-3 bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
        <div class="px-4 py-4 border-b border-gray-100 flex-shrink-0">
          <p class="text-sm font-bold text-gray-900">审核流程</p>
        </div>

        <div class="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          <!-- 时间线 -->
          <div class="space-y-0">
            <div v-for="(step, i) in reviewSteps" :key="i" class="flex gap-3">
              <div class="flex flex-col items-center">
                <div
                  class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 z-10"
                  :class="step.done ? 'bg-blue-600 text-white' : step.current ? 'bg-blue-100 text-blue-600 border-2 border-blue-400' : 'bg-gray-100 text-gray-400'"
                >{{ i + 1 }}</div>
                <div v-if="i < reviewSteps.length - 1" class="w-px flex-1 my-1" :class="step.done ? 'bg-blue-200' : 'bg-gray-100'" style="min-height:28px"></div>
              </div>
              <div class="pb-4 flex-1 min-w-0">
                <div class="flex items-center justify-between gap-1">
                  ...
                  <span
                    ...
                    :class="step.done ? 'bg-green-100 text-green-700' : step.current ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-400'"
                  >{{ step.statusText }}</span>
                </div>
          ...
          <!-- 审核意见 -->
          <div>
            <p class="text-sm font-bold text-gray-800 mb-3">申请意见 <span class="text-red-500">*</span></p>
            <div class="flex gap-4 mb-4">
              <label class="flex items-center gap-2 cursor-pointer" @click="reviewDecision = 'pass'">
                <div
                  class="w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all"
                  :class="reviewDecision === 'pass' ? 'border-blue-600 bg-blue-600' : 'border-gray-300'"
                >
                  <div v-if="reviewDecision === 'pass'" class="w-1.5 h-1.5 rounded-full bg-white"></div>
                </div>
                <span class="text-sm font-medium text-gray-700">通过</span>
              </label>
              <label class="flex items-center gap-2 cursor-pointer" @click="reviewDecision = 'reject'">
                <div
                  class="w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all"
                  :class="reviewDecision === 'reject' ? 'border-red-500 bg-red-500' : 'border-gray-300'"
                >
                  <div v-if="reviewDecision === 'reject'" class="w-1.5 h-1.5 rounded-full bg-white"></div>
                </div>
                <span class="text-sm font-medium text-gray-700">不通过</span>
              </label>
            </div>

            <div>
              <p class="text-sm text-gray-600 mb-1.5">意见说明</p>
              <textarea
                v-model="reviewComment"
                rows="4"
                placeholder="请输入审核意见（选填）"
                maxlength="200"
                class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-700 outline-none focus:border-gray-500 hover:border-gray-400 transition-colors resize-none placeholder-gray-300"
              ></textarea>
              <div class="text-right text-xs text-gray-300 mt-0.5">{{ reviewComment.length }} / 200</div>
            </div>
          </div>
        </div>
...
const activeFundTab = ref('scholarship')
...
const reviewSteps = ref([
  { title: '学生提交申请', statusText: '', done: true, date: '2026-04-02 14:32', operator: '' },
  { title: '辅导员审核', statusText: '已通过', done: true, date: '2026-04-03 00:15', operator: '辅导员：王老师' },
  { title: '学院审核', statusText: '待审核', done: false, current: true, date: '', operator: '当前节点' },
  { title: '学校审核', statusText: '待审核', done: false, current: false, date: '', operator: '' },
  { title: '公示', statusText: '待审核', done: false, current: false, date: '', operator: '' }
])
