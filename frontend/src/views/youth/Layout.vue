  ...
  <div class="antialiased min-h-screen flex text-on-surface">
    <nav class="sidebar-glass hidden md:flex flex-col fixed left-0 top-0 h-full py-8 px-4 w-64 z-50 overflow-y-auto overscroll-contain custom-scrollbar">
      ...
      <ul class="flex flex-col gap-2 font-medium tracking-tight text-[0.875rem]">
        ...
        <li>
          <router-link to="/youth/activities" active-class="!text-emerald-600 font-bold"
            class="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-secondary hover:bg-white/60 group">
        ...
        <li>
          <router-link to="/youth/awards" active-class="!text-emerald-600 font-bold"
            class="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-secondary hover:bg-white/60 group">
      ...
      <header class="flex items-center justify-between px-8 fixed top-0 left-0 md:left-64 right-0 z-40 h-14 bg-surface/70 backdrop-blur-xl border-b border-outline-variant/10">
        ...
        <div class="flex items-center gap-4">
          <div class="relative flex items-center justify-center">
            <button @click.stop="notifOpen = !notifOpen" class="relative text-secondary hover:text-emerald-600 transition-colors p-1.5 rounded-lg hover:bg-surface-container-low flex items-center justify-center">
              <el-icon :size="20"><Bell /></el-icon>
              <span v-if="unreadCount > 0" class="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 bg-emerald-500 rounded-full text-white text-[10px] font-bold flex items-center justify-center px-1 border-2 border-surface">{{ unreadCount }}</span>
            </button>
          ...
          <button @click="showSettings = true" class="text-secondary hover:text-emerald-600 transition-colors p-1.5 rounded-lg hover:bg-surface-container-low flex items-center justify-center">
            <el-icon :size="20"><Setting /></el-icon>
          </button>
          <div class="flex items-center gap-2 cursor-pointer group">
            <div class="w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm shadow-md">团</div>
            <span class="text-sm font-semibold text-on-surface group-hover:text-emerald-600 transition-colors">第二课堂/团委</span>
          </div>
        </div>
      </header>
      ...
      <div class="relative bg-surface rounded-2xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden border border-outline-variant/20">
        ...
        <div class="flex min-h-[460px]">
          <div class="w-48 bg-surface-container-lowest border-r border-outline-variant/15 p-4 flex flex-col">
            ...
            <button @click="settingsTab = 'password'" class="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm text-left mb-1 transition-colors"
              :class="settingsTab === 'password' ? 'bg-emerald-500 text-white font-semibold' : 'text-secondary hover:bg-surface-container-low'">
          ...
          <div class="flex-1 p-8">
            <div v-if="settingsTab === 'password'">
              ...
              <div class="space-y-4 max-w-sm">
                ...
                <button @click="changePwd" class="mt-2 px-6 py-2.5 bg-emerald-500 text-white rounded-xl text-sm font-bold hover:bg-emerald-600 transition-colors shadow-md">确认修改</button>
              </div>
            </div>
          </div>
        </div>
...
const changePwd = () => {
  ...
  ElMessage.success('密码修改成功（演示模式）'); showSettings.value = false; pwdForm.value = { current: '', newPwd: '', confirm: '' }
}
