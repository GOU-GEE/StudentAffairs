...
const changePwd = () => { if (!pwdForm.value.current || !pwdForm.value.newPwd || !pwdForm.value.confirm) { ElMessage.warning('请填写完整的密码信息'); return } if (pwdForm.value.newPwd !== pwdForm.value.confirm) { ElMessage.error('两次输入的新密码不一致'); return } if (pwdForm.value.newPwd.length < 8) { ElMessage.error('新密码至少需要 8 位'); return } ElMessage.success('密码修改成功（演示模式）'); showSettings.value = false; pwdForm.value = { current: '', newPwd: '', confirm: '' } }
